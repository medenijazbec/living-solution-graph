...
function usage() {
  console.error(`Living Solution Graph MCP v6.2.0\n\nUsage:\n  lsg-mcp stdio\n  lsg-mcp http\n  lsg-mcp doctor\n\nEnvironment: see .env.example`);
}
...
async function main() {
  ...
  if (command === 'doctor') {
    ...
    console.log(JSON.stringify({ ok: true, version: '6.2.0', node: process.version, db_path: config.dbPath, starter_packs: packCount }, null, 2));
    closeStore();
    return;
  }
  ...
  if (command === 'http') {
    ...
    console.log(`Living Solution Graph v6.2.0 listening on http://${host}:${port}`);
