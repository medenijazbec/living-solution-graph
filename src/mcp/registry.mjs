...
export function buildRegistry(service){
  ...
  add('solution.add_edge_case','Let the model insert a newly discovered edge case into the graph under a feature. New edge cases start implemented=false and unverified.',obj({project_id:s,parent_feature_id:s,title:s,description:s,category:s,severity:s,disposition:s,reason:s,origin:s,actor:s,expected_graph_version:i},['project_id','parent_feature_id','title','expected_graph_version']),a=>service.addEdgeCase(a));
  ...
  add('solution.stage_semantic_feature_set','Validate and persist a Codex-authored semantic feature/edge-case proposal for review. This stages a proposal but does not modify live graph nodes.',obj({project_id:s,run_id:s,proposal:{type:'object'},max_features:i,actor:s},['project_id','run_id','proposal']),a=>service.stageSemanticFeatureSet(a));
