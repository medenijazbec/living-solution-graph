...
export function buildRegistry(service){
  ...
  add('solution.delete_project','Permanently delete a project and its graph, plans, memory, and history. Requires its exact title for confirmation.',obj({project_id:s,confirm_title:s},['project_id','confirm_title']),a=>service.deleteProject(a));
  ...
  add('solution.update_semantic_edge_case','Edit a semantic edge case while preserving its stable E-number, semantic key, and source links.',obj({project_id:s,node_id:s,expected_graph_version:i,expected_node_version:i,title:s,description:s,trigger:s,expected_behavior:s,validation_scenario:s,severity:s,actor:s},['project_id','node_id','expected_graph_version','expected_node_version']),a=>service.updateSemanticEdgeCase(a));
  add('solution.delete_semantic_edge_case','Remove an edge case from the active semantic backlog while retaining its history and E-number. Requires exact display ID confirmation.',obj({project_id:s,node_id:s,expected_graph_version:i,expected_node_version:i,confirm_display_id:s,actor:s},['project_id','node_id','expected_graph_version','expected_node_version','confirm_display_id']),a=>service.deleteSemanticEdgeCase(a));
  ...
  add('solution.append_node_implementation_plan','Append a Markdown section to a feature or edge-case plan, preserving existing text and document history.',obj({project_id:s,node_id:s,markdown:s,file_name:s,expected_document_version:i,actor:s},['project_id','node_id','markdown']),a=>service.appendNodeImplementationPlan(a));
  add('solution.delete_node_implementation_plan','Delete an attached Markdown plan after exact file-name confirmation. Definition history remains available.',obj({project_id:s,node_id:s,confirm_file_name:s,expected_document_version:i,actor:s},['project_id','node_id','confirm_file_name']),a=>service.deleteNodeImplementationPlan(a));
  add('solution.get_implementation_plan_coverage','Audit active feature and edge-case nodes with and without written Markdown plans; includes default numbered filenames.',obj({project_id:s,missing_only:b,include_source:b},['project_id']),a=>service.getImplementationPlanCoverage(a));
