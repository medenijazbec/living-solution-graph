...
export const SERVER_INFO={name:'living-solution-graph',version:'6.0.0'};
...
const promptDefs=[
  ...
  {name:'provide_semantic_feature_list',description:'Have Codex build a reviewable, source-linked, numbered semantic implementation backlog with nested feature sets and edge cases. Codex treats “use lsg”, “@lsg”, and “run lsg” as an invocation of this server.',arguments:[{name:'project_id',required:true},{name:'max_features',required:false}]}
];
...
export class McpProtocol {
  ...
  async callTool(id,params,modern){
    ...
    try{
      const out=await t.handler(params.arguments||{}); const structured=toStructured(out);
  ...
  getPrompt(id,params,modern){
    ...
    else if(name==='provide_semantic_feature_list') text=`Provide the semantic feature list for project ${a.project_id}. First call solution.prepare_semantic_feature_set with max_features ${a.max_features||25}. Use its source-node catalog and proposal contract to author evidence-linked implementation units, dependencies, acceptance criteria, and edge cases. Decompose broad features into buildable child features using parent_key (for example player.movement with movement.walk, movement.vault, movement.peek, movement.lean, movement.climb, and movement.prone). Call solution.stage_semantic_feature_set, present the numbered staged proposal and diff for review, and do not call solution.commit_semantic_feature_set unless the user explicitly approves the staged result. When a user names F1, F1.2, or 1, call solution.resolve_semantic_feature_reference before staging the requested update. In Codex, the user phrases “use lsg”, “@lsg”, and “run lsg” are explicit invocations of this workflow.`;
