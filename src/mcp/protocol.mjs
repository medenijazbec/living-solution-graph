...
export class McpProtocol {
  ...
  getPrompt(id,params,modern){
    ...
    else if(name==='provide_semantic_feature_list') text=`Provide the semantic feature list for the current workspace. If the user supplied a master-plan path${a.plan_file_path?` (${a.plan_file_path})`:''}, read that plan, call solution.resolve_workspace_project with working_directory set to the plan's containing directory, and use its returned project.id. Import or update the lexical graph with solution.preview_markdown_plan (pass source_file_path as the absolute plan path) and solution.commit_plan_import before semantic preparation. Otherwise resolve working_directory ${a.working_directory||'<current working directory>'}. Call solution.prepare_semantic_feature_set with max_features ${a.max_features||25}. Read importance_scale, author source-linked features and edge cases, and decompose broad features with parent_key. Stage the proposal; it automatically commits unless the user requests review-only. Retrieve solution.get_semantic_feature_list and report each F-number with its recursive edge-case count. Then call solution.get_implementation_plan_coverage; for every missing feature and edge case, author a bounded Markdown implementation plan with solution.set_node_implementation_plan using the default numbered filename, e.g. F26.1.E1-implementation-plan.md. Include outcome, source evidence, dependencies, implementation steps, acceptance checks, and edge-case validation. The MCP server stores plans as attached documents, not files on disk. Use solution.update_semantic_edge_case for edits and solution.delete_semantic_edge_case only with explicit user confirmation. Resolve F-number references before edits.`;
