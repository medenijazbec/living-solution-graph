...
export class McpProtocol {
  ...
  getPrompt(id,params,modern){
    ...
    else if(name==='continue_project') text=`Continue project ${a.project_id}. Start with solution.run_program program=overview or next_work. After choosing a feature, call solution.select_node_neighborhood for its directly connected context; use solution.select_node_cascade only when one or more additional neighbor generations are relevant. Implement one bounded gap at a time, run relevant tests, attach evidence, then update implementation and verification state. Add newly discovered edge cases to the graph instead of leaving them in prose.`;
