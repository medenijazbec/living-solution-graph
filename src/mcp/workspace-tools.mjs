...
export function workspaceTools(service){
  ...
  const importanceLevel={type:'object',properties:{id:{type:'string',minLength:1,maxLength:32,pattern:'^[A-Za-z][A-Za-z0-9_-]{0,31}$'},label:{type:'string',minLength:1,maxLength:100},description:{type:'string',minLength:1,maxLength:500}},required:['id','label','description'],additionalProperties:false};
  const workResult={type:'object',properties:{summary:{type:'string',minLength:1,maxLength:4000},commit_sha:s,changed_files:strings,test_commands:strings,notes:s},required:['summary'],additionalProperties:false};
  const reviewEvidence={type:'object',properties:{kind:{type:'string',enum:['automated_test','manual_verification']},summary:{type:'string',minLength:1,maxLength:4000},command:s,exit_code:i,artifact_uri:s,commit_sha:s,checked_at:s},required:['kind','summary'],additionalProperties:false};
  ...
  add('solution.set_importance_scale','Configure ordered instance importance levels; map any removed levels that are in use.',{project_id:s,levels:{type:'array',items:importanceLevel,minItems:1,maxItems:32},expected_version:{type:'integer',minimum:0},replacements:o,actor:s},['levels','expected_version'],a=>service.workspace.setScale(a));
  ...
  add('solution.update_work','Renew, release or submit work results. Submission does not verify the node.',{project_id:s,work_id:s,owner:s,expected_version:i,action:{type:'string',enum:['renew','release','submit']},result:workResult},['project_id','work_id','owner','expected_version','action'],a=>service.workspace.work(a,'update'));
  add('solution.review_work','Review submitted work with current automated_test or manual_verification evidence.',{project_id:s,work_id:s,expected_version:i,reviewer:s,approved:b,evidence:reviewEvidence},['project_id','work_id','expected_version','reviewer','approved','evidence'],a=>service.workspace.work(a,'review'));
