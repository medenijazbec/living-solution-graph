...
export function workspaceTools(service){
  ...
  add('solution.get_importance_scale','Read ordered instance importance levels and scale version.',{},[],()=>importanceScale(service.store));
  add('solution.set_importance_scale','Configure ordered instance importance levels; map any removed levels that are in use.',{levels:{type:'array',items:o},expected_version:i,replacements:o,actor:s},['levels','expected_version'],a=>service.workspace.setScale(a));
