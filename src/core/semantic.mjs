...
import { importanceScale } from './workspace.mjs';
...
// A semantic proposal is validated and staged before automatic materialization, so its
// user-controlled planning boundary rather than a hidden product constraint.
// Callers may supply any positive safe integer through max_features.
function featureLimit(value, fallback=25){
...
export class SemanticLayer {
  ...
  prepare(input){
    ...
    const scope=input.scope||'project'; const limit=featureLimit(input.max_features); const sourceLimit=Math.min(Math.max(Number(input.source_node_limit||300),25),1000);
  ...
  validateProposal(projectId,proposal,maxFeatures=25){
    ...
    for(const raw of proposal.features){
      ...
      const existing=[...projectNodes.values()].find(n=>n.metadata.semantic_key===key&&n.type==='feature');const priority=existing?.metadata.importance_override?existing.metadata.priority:String(raw.priority||existing?.metadata.priority||''); if(!importanceScale(this.store).levels.some(l=>l.id===priority))fail(`Feature ${key} has invalid priority`);const rationale=existing?.metadata.importance_override?existing.metadata.importance_rationale:String(raw.importance_rationale||existing?.metadata.importance_rationale||'').trim();
      ...
      if(!rationale&&!existing)fail(`Feature ${key} requires importance_rationale`);
      normalized.push({key,parent_key:parentKey,title,outcome,priority,importance_rationale:rationale,proposed_gap:proposedGap,source_node_ids:sourceNodeIds,acceptance_criteria:criteria,non_goals:asArray(raw.non_goals).map(String).filter(Boolean),depends_on:dependencies,edge_cases:cases});
    }
  ...
  stage(input){
    ...
    if(run.brief?.importance_scale&&run.brief.importance_scale.version!==importanceScale(this.store).version)fail('Importance scale changed since preparation; prepare a new run','SEMANTIC_RUN_STALE');
    const proposal=this.validateProposal(project.id,input.proposal,featureLimit(input.max_features)); const existingFeatures=keyMap(semanticNodes(this.store,project.id,{activeOnly:true}).filter(n=>n.type==='feature'&&!n.metadata?.semantic_root));
    const numbering=numberFeatures(proposal.features,existingFeatures);const proposedKeys=new Set(proposal.features.map(f=>f.key)); const diff={created:proposal.features.filter(f=>!existingFeatures.has(f.key)).map(f=>f.key),updated:proposal.features.filter(f=>existingFeatures.has(f.key)).map(f=>f.key),superseded:[...existingFeatures.values()].filter(n=>!proposedKeys.has(n.metadata.semantic_key)).map(n=>({id:n.id,key:n.metadata.semantic_key,number:n.metadata.display_id||null,title:n.title})),edge_case_count:proposal.features.reduce((n,f)=>n+f.edge_cases.length,0),unsupported:proposal.features.filter(f=>f.proposed_gap).map(f=>f.key),numbering:Object.fromEntries(numbering)};
  ...
  commit(input){
    ...
    return this.store.tx(()=>{
      ...
      const byKey=new Map(run.proposal.features.map(f=>[f.key,f]));const materialize=key=>{if(featureByKey.has(key))return featureByKey.get(key);const feature=byKey.get(key);const parent=feature.parent_key?materialize(feature.parent_key):root;const node=upsert('feature',feature.key,feature.title,feature.outcome,parent.id,{display_id:numbering.get(feature.key),parent_semantic_key:feature.parent_key,priority:feature.priority,importance_rationale:feature.importance_rationale,proposed_gap:feature.proposed_gap,source_node_ids:feature.source_node_ids,acceptance_criteria:feature.acceptance_criteria,non_goals:feature.non_goals});featureByKey.set(feature.key,node);return node;};for(const feature of run.proposal.features)materialize(feature.key);
      const pending=[];
      for(const feature of run.proposal.features){const node=featureByKey.get(feature.key);for(let i=0;i<feature.acceptance_criteria.length;i++){const key=`${feature.key}.acceptance.${i+1}`;const test=upsert('test',key,feature.acceptance_criteria[i],`Acceptance criterion for ${feature.title}.`,node.id,{feature_key:feature.key,criterion_index:i+1});pending.push({kind:'validated_by',source_id:node.id,target_id:test.id});}
        for(let i=0;i<feature.edge_cases.length;i++){const edgeCase=feature.edge_cases[i];const ec=upsert('edge_case',edgeCase.key,edgeCase.title,edgeCase.expected_behavior,node.id,{display_id:`${numbering.get(feature.key)}.E${i+1}`,feature_key:feature.key,trigger:edgeCase.trigger,severity:edgeCase.severity,validation_scenario:edgeCase.validation_scenario});pending.push({kind:'has_edge_case',source_id:node.id,target_id:ec.id});}
  ...
  listFeatures(projectId){
    ...
    const directCases=new Map(features.map(feature=>[feature.id,edges.filter(e=>e.type==='has_edge_case'&&e.source_id===feature.id).map(e=>byId.get(e.target_id)).filter(Boolean)]));const children=new Map();for(const feature of features){const parent=feature.metadata?.parent_semantic_key||null;if(!children.has(parent))children.set(parent,[]);children.get(parent).push(feature);}
    const recursiveCount=feature=>{const seen=new Set(),cases=new Set();const visit=f=>{if(seen.has(f.id))return;seen.add(f.id);for(const c of directCases.get(f.id)||[])cases.add(c.id);for(const child of children.get(f.metadata?.semantic_key)||[])visit(child);};visit(feature);return cases.size;};
    const mapped=features.map(feature=>{const direct=directCases.get(feature.id)||[];const recursive=recursiveCount(feature);return {...feature,dependencies:edges.filter(e=>e.type==='depends_on'&&e.source_id===feature.id).map(e=>byId.get(e.target_id)).filter(Boolean).map(n=>({id:n.id,key:n.metadata.semantic_key,title:n.title,implemented:n.implemented})),edge_cases:direct,acceptance_tests:edges.filter(e=>e.type==='validated_by'&&e.source_id===feature.id).map(e=>byId.get(e.target_id)).filter(Boolean).sort((a,b)=>(a.metadata?.criterion_index||0)-(b.metadata?.criterion_index||0)),direct_edge_case_count:direct.length,recursive_edge_case_count:recursive,descendant_feature_count:features.filter(candidate=>candidate.metadata?.display_id?.startsWith(`${feature.metadata?.display_id}.`)).length,count_label:`${feature.metadata?.display_id||feature.title} → ${recursive} EC`};});
    return {project_id:project.id,graph_version:project.graph_version,counts:{features_total:features.length,root_features:features.filter(f=>!f.metadata?.parent_semantic_key).length,subfeatures:features.filter(f=>!!f.metadata?.parent_semantic_key).length,edge_cases_total:nodes.filter(n=>n.type==='edge_case').length},features:mapped};
  }

  updateFeature(input){
    const project=this.store.requireProject(input.project_id);this.store.assertGraphVersion(project.id,input.expected_graph_version);const node=this.store.getNode(input.node_id);if(!node||node.project_id!==project.id||node.type!=='feature'||node.metadata?.layer!=='semantic'||node.metadata?.semantic_root)fail('Semantic feature not found','SEMANTIC_FEATURE_NOT_FOUND');
    if(input.expected_node_version!=null&&node.version!==input.expected_node_version)fail(`Node version conflict: expected ${input.expected_node_version}, current ${node.version}`,'NODE_VERSION_CONFLICT',{current_node_version:node.version});
    const title=String(input.title??node.title).trim();const outcome=String(input.outcome??node.description).trim();const priority=String(input.priority??node.metadata?.priority??'P2');if(!title||!outcome)fail('Semantic feature title and outcome are required');if(!importanceScale(this.store).levels.some(l=>l.id===priority))fail('Invalid semantic feature priority');
    const criteria=input.acceptance_criteria==null?(node.metadata?.acceptance_criteria||[]):asArray(input.acceptance_criteria).map(x=>String(x).trim()).filter(Boolean);if(!criteria.length)fail('Semantic feature requires at least one acceptance criterion');const nonGoals=input.non_goals==null?(node.metadata?.non_goals||[]):asArray(input.non_goals).map(x=>String(x).trim()).filter(Boolean);
    this.store.tx(()=>{this.store.updateNodeInTransaction(node.id,{title,description:outcome,metadata:{priority,importance_override:true,acceptance_criteria:criteria,non_goals:nonGoals}},input.expected_node_version??null);const tests=this.store.listNodes(project.id,{types:['test']}).filter(test=>test.parent_id===node.id&&test.metadata?.layer==='semantic').sort((a,b)=>(a.metadata?.criterion_index||0)-(b.metadata?.criterion_index||0));for(let i=0;i<criteria.length;i++){let test=tests[i];const metadata={layer:'semantic',semantic_key:`${node.metadata.semantic_key}.acceptance.${i+1}`,semantic_run_id:node.metadata.semantic_run_id,feature_key:node.metadata.semantic_key,criterion_index:i+1};if(test)test=this.store.updateNodeInTransaction(test.id,{title:criteria[i],description:`Acceptance criterion for ${title}.`,status:'active',metadata});else{test=this.store.insertNode({project_id:project.id,type:'test',title:criteria[i],description:`Acceptance criterion for ${title}.`,origin:'semantic_codex',status:'active',parent_id:node.id,metadata});this.store.insertEdge({project_id:project.id,source_id:node.id,target_id:test.id,type:'validated_by',origin:'semantic_codex'});}}for(const extra of tests.slice(criteria.length))this.store.deleteNodeInTransaction(extra.id);this.store.bumpGraphVersion(project.id);this.store.event({project_id:project.id,kind:'semantic.feature_updated',actor:input.actor||'user',entity_id:node.id,data:{title,priority,acceptance_criteria:criteria.length}});});
    return this.listFeatures(project.id).features.find(feature=>feature.id===node.id);
  }
