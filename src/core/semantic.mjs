...
export class SemanticLayer {
  ...
  validateProposal(projectId,proposal,maxFeatures=25){
    ...
    for(const raw of proposal.features){
      ...
      const dependencies=asArray(raw.depends_on).map(String); const cases=[]; const caseKeys=new Set();
      ...
      normalized.push({key,title,outcome,priority,proposed_gap:proposedGap,source_node_ids:sourceNodeIds,acceptance_criteria:criteria,non_goals:asArray(raw.non_goals).map(String).filter(Boolean),depends_on:dependencies,edge_cases:cases});
    }
    for(const feature of normalized)for(const dep of feature.depends_on)if(!keys.has(dep))fail(`Feature ${feature.key} depends on unknown proposal key ${dep}`);
    const visiting=new Set(),visited=new Set(),byKey=new Map(normalized.map(f=>[f.key,f])); const visit=key=>{if(visiting.has(key))fail(`Semantic dependency cycle includes ${key}`,'SEMANTIC_DEPENDENCY_CYCLE');if(visited.has(key))return;visiting.add(key);for(const dep of byKey.get(key).depends_on)visit(dep);visiting.delete(key);visited.add(key);}; normalized.forEach(f=>visit(f.key));
  ...
  stage(input){
    ...
    const proposedKeys=new Set(proposal.features.map(f=>f.key)); const diff={created:proposal.features.filter(f=>!existing.has(f.key)).map(f=>f.key),updated:proposal.features.filter(f=>existing.has(f.key)).map(f=>f.key),superseded:[...existing.values()].filter(n=>!n.metadata.semantic_root&&!proposedKeys.has(n.metadata.semantic_key)).map(n=>({id:n.id,key:n.metadata.semantic_key,title:n.title})),edge_case_count:proposal.features.reduce((n,f)=>n+f.edge_cases.length,0),unsupported:proposal.features.filter(f=>f.proposed_gap).map(f=>f.key)};
  ...
  commit(input){
    ...
    return this.store.tx(()=>{
      ...
      const desired=new Set(['semantic.backlog']);const featureByKey=new Map();const created=[];const reused=[];
      ...
      for(const feature of run.proposal.features){const node=upsert('feature',feature.key,feature.title,feature.outcome,root.id,{priority:feature.priority,proposed_gap:feature.proposed_gap,source_node_ids:feature.source_node_ids,acceptance_criteria:feature.acceptance_criteria,non_goals:feature.non_goals});featureByKey.set(feature.key,node);}
      const pending=[];
  ...
  listFeatures(projectId){
    const project=this.store.requireProject(projectId);const nodes=semanticNodes(this.store,project.id,{activeOnly:true});const features=nodes.filter(n=>n.type==='feature'&&!n.metadata?.semantic_root).sort(priorityOrder);const edges=this.store.listEdges(project.id);const byId=new Map(nodes.map(n=>[n.id,n]));
