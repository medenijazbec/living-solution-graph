...
export class SemanticLayer {
  ...
  listFeatures(projectId){
    ...
    const recursiveCount=(feature,seen=new Set())=>{if(seen.has(feature.id))return 0;seen.add(feature.id);let total=directCases.get(feature.id)?.length||0;for(const child of children.get(feature.metadata?.semantic_key)||[])total+=recursiveCount(child,seen);return total;};
  ...
  updateFeature(input){
    ...
    const title=String(input.title??node.title).trim();const outcome=String(input.outcome??node.description).trim();const priority=String(input.priority??node.metadata?.priority??'P2');if(!title||!outcome)fail('Semantic feature title and outcome are required');if(!priorities.includes(priority))fail('Invalid semantic feature priority');
