...
export class SemanticLayer {
  ...
  stage(input){
    ...
    const numbering=numberFeatures(proposal.features,existing);const proposedKeys=new Set(proposal.features.map(f=>f.key)); const diff={created:proposal.features.filter(f=>!existing.has(f.key)).map(f=>({key:f.key,number:numbering.get(f.key)})),updated:proposal.features.filter(f=>existing.has(f.key)).map(f=>({key:f.key,number:numbering.get(f.key)})),superseded:[...existing.values()].filter(n=>!n.metadata.semantic_root&&!proposedKeys.has(n.metadata.semantic_key)).map(n=>({id:n.id,key:n.metadata.semantic_key,number:n.metadata.display_id||null,title:n.title})),edge_case_count:proposal.features.reduce((n,f)=>n+f.edge_cases.length,0),unsupported:proposal.features.filter(f=>f.proposed_gap).map(f=>f.key),numbering:Object.fromEntries(numbering)};
