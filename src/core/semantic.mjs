...
export class SemanticLayer {
  ...
  stage(input){
    ...
    const proposal=this.validateProposal(project.id,input.proposal,Math.min(Math.max(Number(input.max_features||25),1),100)); const existingFeatures=keyMap(semanticNodes(this.store,project.id,{activeOnly:true}).filter(n=>n.type==='feature'&&!n.metadata?.semantic_root));
