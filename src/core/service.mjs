...
export class LsgService {
  ...
  validateProjectIntegrity(input) {
    ...
    const fileRows=this.store.db.prepare("SELECT id,data FROM workspace_records WHERE project_id=? AND kind='files'").all(project.id);for(const row of fileRows){let data;try{data=JSON.parse(row.data);}catch{add('error','invalid_file_mapping','File mapping is not valid JSON.',{record_id:row.id});continue;}if(!data||typeof data!=='object'||Array.isArray(data)||!Array.isArray(data.paths)){add('error','invalid_file_mapping','File mapping must be an object containing a paths array.',{record_id:row.id});continue;}if(data.project_id&&data.project_id!==project.id)add('error','cross_project_file_mapping','File mapping declares a different project.',{record_id:row.id,mapped_project_id:data.project_id});if(row.id!==data.node_id)add('error','inconsistent_file_mapping','File mapping record ID and node ID differ.',{record_id:row.id,node_id:data.node_id||null});if(!byId.has(data.node_id))add('error','orphan_file_mapping','File mapping references a missing node.',{record_id:row.id,node_id:data.node_id||null});for(const mapped of data.paths){try{this.activity.safe(project.id,mapped,{missing:true});}catch(error){add('error','unsafe_file_mapping','Mapped path is unavailable, excluded, linked, or outside the bound workspace.',{record_id:row.id,path:mapped,reason:error.code||'INVALID_PATH'});}}}
