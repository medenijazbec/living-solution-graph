...
export class LsgService {
  ...
  runCompactProgram(input){
    ...
    if(program==='overview'){const {items,...progress}=this.workspace.progress({project_id:project.id});const {items:plans,...coverage}=this.getImplementationPlanCoverage({project_id:project.id});return {program,project:{id:project.id,title:project.title,graph_version:project.graph_version},counts:progress,plans:coverage,next:['feature','next_work','missing_plans','search']};}
