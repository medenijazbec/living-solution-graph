...
try{
  ...
  assert.equal(await noOverlap(),true);assert.equal(await page.locator('#audit .auditCard b').first().textContent(),'80');assert.ok(await page.locator('.worldEdges path').count()>0);assert.equal(await page.locator('#activityToggle').isChecked(),false);
