...
The semantic canvas opens in **Priority orbits**: P0 features form the innermost loose, staggered bands, followed by lower importance levels; related edge cases occupy adjacent outer bands. The guides are intentionally irregular, not rigid circles. The **Columns** selector restores the hierarchy layout. Manual card placement and zoom are saved locally per project, view, and layout. **Enable live activity** is off by default; when enabled, mapped-file changes pulse affected cards and edges and show a temporary **LIVING** indicator. It records no file contents or durable activity history.
