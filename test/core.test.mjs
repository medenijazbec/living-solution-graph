...
test('Codex semantic feature sets stage, commit, drive the frontier, and become stale after a plan update',()=>{
  const r=runtime();try{
    ...
    const staged=r.service.stageSemanticFeatureSet({project_id:p.id,run_id:prepared.run_id,proposal});assert.equal(staged.status,'staged');assert.equal(staged.diff.created.length,2);assert.deepEqual(staged.diff.superseded,[]);
