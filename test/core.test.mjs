...
test('human edge-case edits and deletions survive a later semantic regeneration',()=>{
  const r=runtime();try{let p=r.service.createProject({project_id:'case-regeneration',title:'Case regeneration'});const baseline=r.service.previewMarkdownPlan({project_id:p.id,markdown:'# Product\n- Build cockpit',expected_graph_version:p.graph_version,starter_pack_mode:'disabled',allow_model_edge_case_expansion:false});r.service.commitPlanImport({project_id:p.id,import_session_id:baseline.id,expected_graph_version:baseline.expected_graph_version});const source=r.store.listNodes(p.id).find(n=>n.metadata.layer==='source');const proposal={features:[{key:'cockpit',title:'Cockpit',priority:'P1',importance_rationale:'Core game interaction',outcome:'Pilot can fly',source_node_ids:[source.id],acceptance_criteria:['Pilot enters'],non_goals:[],depends_on:[],edge_cases:[{key:'cockpit.ec1',title:'Old collision',trigger:'Old trigger',expected_behavior:'Old behavior',severity:'high',validation_scenario:'Old check'},{key:'cockpit.ec2',title:'Delete me',trigger:'Bad state',expected_behavior:'Recover',severity:'medium',validation_scenario:'Exercise bad state'}]}]};let run=r.service.prepareSemanticFeatureSet({project_id:p.id});r.service.stageSemanticFeatureSet({project_id:p.id,run_id:run.run_id,proposal});let cases=r.service.getSemanticEdgeCases({project_id:p.id}).edge_cases;let ec=cases[0];r.service.updateSemanticEdgeCase({project_id:p.id,node_id:ec.id,expected_graph_version:r.store.getProject(p.id).graph_version,expected_node_version:ec.version,title:'Human collision',trigger:'Human trigger'});ec=cases[1];r.service.deleteSemanticEdgeCase({project_id:p.id,node_id:ec.id,expected_graph_version:r.store.getProject(p.id).graph_version,expected_node_version:ec.version,confirm_display_id:ec.metadata.display_id});run=r.service.prepareSemanticFeatureSet({project_id:p.id});r.service.stageSemanticFeatureSet({project_id:p.id,run_id:run.run_id,proposal});cases=r.service.getSemanticEdgeCases({project_id:p.id}).edge_cases;assert.equal(cases.length,1);assert.equal(cases[0].title,'Human collision');assert.equal(cases[0].metadata.trigger,'Human trigger');assert.equal(cases[0].id,r.store.getNode(cases[0].id).id);assert.equal(r.store.getNode(ec.id).status,'deleted');}finally{r.close();}
});
