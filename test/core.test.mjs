...
test('Codex semantic feature sets stage, commit, drive the frontier, and become stale after a plan update',()=>{
  const r=runtime();try{
    ...
    const semantic=r.service.getSemanticFeatureList({project_id:p.id});assert.equal(semantic.features.length,2);assert.equal(semantic.features[0].metadata.display_id,'F1');assert.equal(semantic.features[1].metadata.display_id,'F1.1');assert.equal(semantic.features[0].edge_cases.length,1);
