...
test('Codex semantic feature sets stage, commit, drive the frontier, and become stale after a plan update',()=>{
  const r=runtime();try{
    ...
    const proposal={features:[
      ...
      {key:'cockpit.coop',title:'Cockpit co-op control routing',priority:'P1',outcome:'Crew members share bounded control channels.',source_node_ids:[sourceNodes[1].id],acceptance_criteria:['Pilot and gunner can control separate channels.'],non_goals:[],depends_on:['cockpit.foundation'],edge_cases:[]}
    ]};
    const staged=r.service.stageSemanticFeatureSet({project_id:p.id,run_id:prepared.run_id,proposal});assert.equal(staged.status,'staged');assert.equal(staged.diff.created.length,2);
    ...
    const semantic=r.service.getSemanticFeatureList({project_id:p.id});assert.equal(semantic.features.length,2);assert.equal(semantic.features[0].edge_cases.length,1);
