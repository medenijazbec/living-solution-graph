...
test('modern 2026 MCP discover/list/call/read resource',async()=>{const r=rt();try{
  let x=await r.protocol.handle(modern(1,'server/discover'),{era:'modern'});assert.equal(x.result._meta['io.modelcontextprotocol/serverInfo'].version,'6.0.0');assert.equal(x.result.resultType,'complete');assert.deepEqual(x.result.supportedVersions,[MODERN_PROTOCOL]);
  x=await r.protocol.handle(modern(2,'tools/list'),{era:'modern'});assert.ok(x.result.tools.some(t=>t.name==='solution.add_edge_case'));assert.ok(x.result.tools.some(t=>t.name==='solution.audit_implementation_status'));assert.ok(x.result.tools.some(t=>t.name==='solution.prepare_semantic_feature_set'));
