...
test('modern 2026 MCP discover/list/call/read resource',async()=>{const r=rt();try{
  let x=await r.protocol.handle(modern(1,'server/discover'),{era:'modern'});assert.equal(x.result._meta['io.modelcontextprotocol/serverInfo'].version,'6.2.0');assert.equal(x.result.resultType,'complete');assert.deepEqual(x.result.supportedVersions,[MODERN_PROTOCOL]);
